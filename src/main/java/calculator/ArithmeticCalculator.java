...
public class ArithmeticCalculator<T extends Number, R extends Number> {
    //덧셈
    private final AddOperator<T, R> addOperator = new AddOperator<>();
    //뺄셈
    private final DivideOperator<T, R> divideOperator = new DivideOperator();
    //곱셈
    private final MultiplyOperator<T, R> multiplyOperator = new MultiplyOperator();
    //나눗셈
    private final SubtractOperator<T, R> subtractOperator = new SubtractOperator();
    //나머지
    private final ModOperator<T, R> modOperator = new ModOperator();
    ...
    public R calculate(T x, String operator, T y) throws Exception {
        ...
        try {
            ...
            //분모확인
            if (y.equals(0L)) throw new HandleArithmeticException(y);
